...
function scheduleSpawn() {
  setTimeout(() => {
    if (Game.customers.length < Game.maxSeats) {
      spawnCustomer();
    } else {
      logEvent('All tables are full! Serve faster to open space.', 'warning');
    }
    Game.spawnInterval = Math.max(Game.minSpawn, Game.spawnInterval * 0.96);
    Game.rushLevel = Math.min(8, 1 + Math.floor((Date.now() - Game.startTime) / 20000));
    scheduleSpawn();
  }, Game.spawnInterval);
}
...
function spawnCustomer() {
  ...
  const base = 12000 + Math.random() * 9000;
  const bonusRush = Math.max(0, (Game.rushLevel - 1) * -600);
  const remainingTime = base + bonusRush;
  const customer = {
    id: Game.nextId++,
    order,
    remainingTime,
  ...
  logEvent(`Customer ${customer.id} slides in and orders ${order.map((o) => o.icon).join(' ')}`);
  renderUI();
}
...
function handleDeparture(customer) {
  Game.score -= 10;
  Game.streak = 0;
  logEvent(`Customer ${customer.id} storms out! Order lost. (-10)`, 'warning');
...
function handleServe() {
  ...
  if (correct && customer.remainingTime > 0) {
    const speedBonus = Math.max(0, Math.floor((customer.remainingTime / customer.maxTime) * 6));
    Game.streak += 1;
    Game.bestStreak = Math.max(Game.bestStreak, Game.streak);
    const tip = Math.floor(Math.random() * 4) + Game.streak;
    const rushBonus = Game.rushLevel;
    const points = 12 + tip + speedBonus + rushBonus;
    Game.score += points;
    logEvent(
      `Served Customer ${customer.id} perfectly! (+${points} | tip +${tip} 💵 | streak x${Game.streak})`,
      'tip'
    );
    ...
    Game.score -= 7;
    Game.streak = 0;
    logEvent(`Wrong order for Customer ${customer.id}. (-7)`, 'warning');
  }
...
function hypeLabel() {
  if (Game.streak >= 6) return 'Blazing';
  if (Game.streak >= 3) return 'Warm';
  if (Game.rushLevel >= 6) return 'Chaos';
  if (Game.rushLevel >= 3) return 'Busy';
  return 'Calm';
}
...
function renderUI() {
  ...
  document.getElementById('rushLevel').textContent = `Rush Lv. ${Game.rushLevel}`;
  document.getElementById('streak').textContent = `Streak: ${Game.streak}🔥`;
  document.getElementById('bestStreak').textContent = Game.bestStreak;
  document.getElementById('seatCount').textContent = `${Game.customers.length}/${Game.maxSeats}`;
  document.getElementById('hype').textContent = hypeLabel();
  renderCustomers();
  renderTray();
}
...
function renderCustomers() {
  ...
  Game.customers.forEach((customer) => {
    ...
    const urgency = Math.max(0.6, customer.remainingTime / customer.maxTime);
    row.style.transform = `translateZ(${40 * urgency}px) rotateX(${6 * urgency}deg)`;
