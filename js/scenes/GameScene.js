class GameScene extends Phaser.Scene {
  ...
  textureKey(base) {
    const baked = `${base}_iso`;
    const glow = `${base}_glow`;
    if (this.textures.exists(baked)) return baked;
    if (this.textures.exists(glow)) return glow;
    return this.textures.exists(base) ? base : 'panel';
  }
  ...
  createWorld() {
    ...
    for (let x = 0; x < this.gridWidth; x++) {
      for (let y = 0; y < this.gridHeight; y++) {
        ...
        const floor = this.add
          .image(pos.x, pos.y, this.textureKey('floor'))
          .setDisplaySize(this.tileWidth, this.tileHeight);
        floor.setDepth(pos.y);
        if (y === 0) {
          const wall = this.add
            .image(pos.x, pos.y - this.tileHeight / 1.5, this.textureKey('wall'))
            .setDisplaySize(this.tileWidth, this.tileHeight * 1.2);
    ...
    const counter = this.add
      .image(kitchenPos.x, kitchenPos.y - 8, this.textureKey('kitchen'))
      .setDisplaySize(this.tileWidth * 1.7, this.tileHeight * 2);
  ...
  spawnCustomer() {
    ...
    const spriteKey = Phaser.Math.Between(0, 1) === 0 ? this.textureKey('customer1') : this.textureKey('customer2');
